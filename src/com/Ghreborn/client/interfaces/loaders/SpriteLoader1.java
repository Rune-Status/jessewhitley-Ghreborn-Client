...
public class SpriteLoader1 {
	...
	public static void loadSprites() {
		try {
			Buffer index = new Buffer(FileOperations.ReadFile(SignLink.findcachedir() + "media_archives/media_archive1.idx"));
			Buffer data = new Buffer(FileOperations.ReadFile(SignLink.findcachedir() + "media_archives/media_archive1.dat"));
...
/*		if (Constants.DUMP_SPRITES) {
			File directory = new File(SignLink.findcachedir() + "Sprites/dump/");
			if (!directory.exists()) {
				directory.mkdir();
			}
			DataUtils.writeFile(
