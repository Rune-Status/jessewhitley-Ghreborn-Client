package com.Ghreborn.client.io.jaggrab;
...
import com.Ghreborn.client.FileUtils;
...
public class JagGrab extends JagGrabConstants {
	...
	public static void requestCrcs() {
		while(!crcsLoaded() && !Main.instance.exitRequested) {

			try (DataInputStream in = openJagGrabRequest("crc")) {

				Main.instance.drawLoadingText(20, "Requesting CRCs..");
	...
	private static void requestPreload(int i, String fileName) {
		...
		while(buffer == null && !Main.instance.exitRequested) {
			Main.instance.drawLoadingText(20, "Requesting " + fileName);
	...
	public static void error(String error) {
		...
		for (int remaining = delay; remaining > 0; remaining--) {
			if (failedAttempts >= 10) {
				Main.instance.drawLoadingText(10, "Game updated - please reload page");
				remaining = 10;
			} else {
				Main.instance.drawLoadingText(10, ""+error+" Error - Retrying in " + remaining + " seconds.");
			}
			try {
				Thread.sleep(1000L);
	...
	public static DataInputStream openJagGrabRequest(String filePath) throws IOException {
		...
		socket = Main.instance.openSocket(FILE_SERVER_PORT);
